...
class DummyVoiceDetector:
    ...
    def detect_voice(self, video_path: Path):
        return self.mapping[str(video_path)]
...
def test_voice_pattern_sync():
    """Case 3: voice-pattern sync."""
    ref = Path("camera1.mp4")
    tgt = Path("camera2.mp4")
    mapping = {
        str(ref): [(0.0, 4.0)],
    ...
    matcher = SyncMatcher(
        ...
        voice_detector=DummyVoiceDetector(mapping),
        resolution=1.0,
    )
    result = matcher.find_sync_offset(ref, tgt, mode="voice", max_offset=3.0)
    assert result["confidence"] > 0.5
